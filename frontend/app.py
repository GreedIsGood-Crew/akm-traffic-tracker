...
async def postback_receive(click_id: str, status: str, payout: str, request: Request,
    ...
    async with pg.acquire() as conn:
        ...
        if True:
            ...
            # log_track(row)
            if row and row["config"] and row["config"] != "null":
...
async def do_campaign_execution(campaign, request: Request) -> Response:
    ...
    config_raw = campaign.get("config")
    if config_raw and config_raw != "null" and isinstance(config_raw, str):
...
async def track_event(campaign, request: Request):
    ...
    # Стартовая запись
    result_row = {
        "campaign_id": campaign["id"]  # Keep as int for Postgres
    }
    ...
    try:
        ...
        # ClickHouse insert disabled - saving to Postgres instead
        
        # Generate click_id and save to DB
        result_row["click_id"] = meta_data.get("click_id") or generate_click_id()
        await save_click_to_db(result_row)
        log_track(f"✅ Click saved to Postgres: {campaign_alias}")
    except Exception as e:
        log_track(f"❌ Click save failed: {str(e)}")
